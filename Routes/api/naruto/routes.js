...
var Naruto_Duel = require("../../../Models/naruto/naruto_duel");
var Naruto_Tournament=require("../../../Models/naruto/naruto_tournament");
var User = require("../../../Models/user/user");



router.post("/naruto/gettournamentinfo",async (req,res)=>
{
    try
    {   
       console.log("/naruto/gettournamentinfo");
       
           var tournament=await Naruto_Tournament.findById(ObjectId(req.body.id));
           if(tournament)
           {
                await tournament.execPopulate('auction');
                await tournament.execPopulate('narutocharacters');
...
router.post("/naruto/createcharacters",async (req,res)=>
{
    var array=req.body;
    ...
    try
    {
        array.map(async (x)=>
        {
            const character=await Naruto_Character.create(x);
...
router.post("/naruto/createtournament",async (req,res)=>
{
    data=req.body;
    try
    {
        if(data.numberofplayers==2)
        {
            ...
            var d= await Naruto_Duel.create(duelobj);
            var tournamentobj=
            {
            ...
            var x=await Naruto_Tournament.create(tournamentobj);
        ...
        if(data.numberofplayers==4)
        {
            ...
            var firstmatch= await Naruto_Duel.create(match1);
            var secondmatch= await Naruto_Duel.create(match2);
            var finalmatch= await Naruto_Duel.create(match3);
            ...
            var fourplayertournament=await Naruto_Tournament.create(tournamentobj); //name will be changed ,now it is two only for testing
...
router.post("/naruto/jointournament",async (req,res)=>
{
    try
    {
        ...
        var data=req.body;
            ...
            var tournament=await Naruto_Tournament.findById(ObjectId(data.joiningid));
           ...
           // await tournament.execPopulate('duels');
            ...
            {
                ischanged=false;
               var obj=await Naruto_Duel.findById(tournament.duels[i]);
              // console.log("HHHHH the duel is : ",obj);
                ...
                if(ischanged)
                {
                    await Naruto_Duel.findByIdAndUpdate(tournament.duels[i],obj);
                    break;
                }
...
router.post("/naruto/updateuser/auctiondata",async (req,res)=>
{
    try
    {
...
router.post("/naruto/auction/buycharacter",async (req,res)=>
{
    try
    {
        var data=req.body;
        ...
        if(user)
        {
            user.Ongoing_Naruto_Deck.push(ObjectId(data.characterid));
            await user.save();

           // var x=await User.findById(ObjectId(data.usermongoid));
            await user.execPopulate('Ongoing_Naruto_Deck');
           // console.log(x);
            res.status(200).json(user);
        }
        else
...
router.post("/naruto/duel/updateuser",async (req,res)=>
{
    try 
    {
        var data=req.body;
...
router.post("/naruto/tournamentwon",async (req,res)=>
{
    try  //will have to change the model name afterwards for 4 player type
    {
        var data=req.body;
        console.log("updating ",data);
        await Naruto_Tournament.findByIdAndUpdate(data.tournament_id,
            {
        ...
        await Naruto_Duel.findByIdAndUpdate(ObjectId(data.duel_id),
        {
            winner:
            {
                name:data.user.Name,
...
router.post("/naruto/duel/nextmatch",async (req,res)=>
{
    try
    {
        var data=req.body;
        ...
        await Naruto_Duel.findByIdAndUpdate(ObjectId(data.winning_duel_id),{
            winner:{
                name:data.user.Name,
        ...
        var duelobj=await Naruto_Duel.findById(data.duel_id);
        if(duelobj)
        {
            ...
            var duel=await Naruto_Duel.findByIdAndUpdate(data.duel_id,duelobj,{new:true});
